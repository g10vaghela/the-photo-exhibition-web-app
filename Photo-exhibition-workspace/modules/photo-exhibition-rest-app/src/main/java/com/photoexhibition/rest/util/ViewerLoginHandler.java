...
public class ViewerLoginHandler {
	...
	public static JSONArray handleViewerLogin(String mobileNumber, String deviceNumber) {
		...
		try {
			log.debug("Getting viewer details for Mobile number :: "+mobileNumber+" and device number :: "+deviceNumber);
			if(Validate.isValidPhoneNumber(mobileNumber) && Validate.isValidDeviceNumber(deviceNumber)){
				...
				log.debug("Is Gegistered Viewer :"+Validator.isNotNull(viewerInfo));
				if(Validator.isNotNull(viewerInfo)) {
					log.debug("Gegistered viewer found now checking for mobile number verification");
					log.debug("Is viewer mobile number : "+mobileNumber+" verified ? "+viewerInfo.isOtpVerified());
					//Registered viewer found
					if(viewerInfo.isOtpVerified()) {
						log.debug("Mobile number is verified for viewer");
						//viewer is registered and mobile number is verified
						responseJsonArray = ViewerDataHandler.getViewerResponseJson(viewerInfo);
					} else {
						log.debug("Mobile number not verified");
					...
					log.debug("Viewer not found with given mobile number and device");
					viewerInfo = viewerInfoService.getViewerInfoByMobileNumber(mobileNumber);
					log.debug("Checking whether mobile number :"+mobileNumber+" is registered with other device ?");
					if(Validator.isNotNull(viewerInfo)) {
						log.debug("Yes, Mobile number is registered with other device number");
						...
						log.debug("No, Mobile number not registered");
						viewerInfo = viewerInfoService.getViewerInfoByDeviceNumber(deviceNumber);
						log.debug("Checking whether device number : "+deviceNumber+" is registered with other mobile number ?");
						if(Validator.isNotNull(viewerInfo)) {
							log.debug("Yes, Device is Registered with other mobile number");
							...
							log.debug("No, This is new viewer");
							...
							log.debug("Creating new viewer in the system");
							responseJsonArray = getViewerOtpJson(viewerInfo);
						}
					}
				}
			} else {
				log.debug("Invalid mobile number or device number");
	...
	public static JSONArray getViewerOtpJson(ViewerInfo viewerInfo) {
		JSONArray responseJsonArray = JSONFactoryUtil.createJSONArray();
		log.debug("Otp verification process started...");
		try {
			...
			log.debug("isOtpServiceOn :: "+isOtpServiceOn);
			JSONObject viewerJsonWrapper = JSONFactoryUtil.createJSONObject();
			JSONObject viewerJson = JSONFactoryUtil.createJSONObject();
			if(isOtpServiceOn) {
				log.debug("Yes, Otp service is on, now generating otp");
				String otpString = OtpGeneratorUtil.generateOtp();
				log.debug("otpString :: "+otpString);
				viewerInfo.setOtp(otpString);
				viewerInfoService.saveOrUpdate(viewerInfo);
				log.debug("Otp saved for user, now sending otp");
				boolean isOtpSent = sendOtp(viewerInfo);
				log.debug("isOtpSent :: "+isOtpSent);
				if(isOtpSent) {
					log.debug("Yes, Otp is sent from system to registered mobile number");
					...
					log.debug("No, There is problem while sending otp");
				...
				log.debug("No, Otp service off, So bypassing otp verification");
